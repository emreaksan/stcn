"""
Script for generating TIMIT dataset for SRNN model.
Taken from https://github.com/marcofraccaro/srnn/blob/master/timit_for_srnn.py

It is modified to run in Python 3. If you would like to get the data in SRNN representation run
> python3 timit_for_srnn.py
Don't forget to set dataset folder (TIMIT_DIR).
...
def reorder(data_in, batch_size, model_seq_len, dtype='float32'):
    ...
    data_resize = int((data_in.shape[0] // (batch_size * model_seq_len)) * model_seq_len * batch_size)
    n_samples = int(data_resize // (model_seq_len))
...
def create_test_set(x_lst):
    n = len(x_lst)
    x_lens = np.array([x.shape[0] for x in x_lst])
    max_len = x_lens.max() - 1
...
def load_wav_files(files):
    ...
    for i, wav_file in enumerate(files):
        print(i, wav_file)
        wav_files += [librosa.load(wav_file, sr=SAMPLINGRATE)[0]]
    return wav_files


def load_wav_files_relative_path(data_dir, files):
    wav_files = []
...
def create_timit_samples(train_vector, valid_vector, test_vector_lst):
    assert len(valid_vector) + len(train_vector) == 4620
    assert len(test_vector_lst) == 1680

    print("MEAN LEN TRAIN", np.mean(np.array([x.shape[0] for x in train_vector])))
    print("MEAN LEN TEST", np.mean(np.array([x.shape[0] for x in test_vector_lst])))
    print("MEAN LEN VALID", np.mean(np.array([x.shape[0] for x in valid_vector])))
    ...
    test_vector_lst = [make_muliple(vec, OUTDIM).reshape((-1, OUTDIM)) for vec in test_vector_lst]
    assert len(test_vector_lst) == 1680
    ...
    return u_train_vector, u_valid_vector, u_test_vector, x_train_vector, x_valid_vector, x_test_vector, mask_test, m, sd


def split_timit_dataset(data_folder):
    print(SEQ_LEN)
    with open(os.path.join('srnn_timit_train_files.txt'), 'rb') as f:
        train_control = [l.rstrip() for l in f]
    with open(os.path.join('srnn_timit_test_files.txt'), 'rb') as f:
        test_control = [l.rstrip() for l in f]

    assert len(train_control) == 4620
    assert len(test_control) == 1680

    train_files = get_files(os.path.join(data_folder, 'TRAIN'), '*.WAV')
    test_split_files = get_files(os.path.join(data_folder, 'TEST'), '*.WAV')
    ...
    return create_timit_samples(train_vector, valid_vector, test_vector_lst)


def srnn_main(data_path):
    u_train_vector, u_valid_vector, u_test_vector, x_train_vector, x_valid_vector, x_test_vector, mask_test, _, _ = create_timit_samples(data_path)
