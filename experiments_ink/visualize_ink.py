...
def draw_stroke_cv2_colored(strokes_abs, size=(120, 1200), margin=(10, 20), pen_width=4, factor=1, color_labels=None,
                            black_zero=True, output_path=None):
    """
    Draws strokes that are used in network training. The pen information consists of 2 possible values namely 0 and 1.
    Args:
        strokes_abs:
...
def draw_stroke_svg(data, factor=0.01, svg_filename='sample.svg', color_labels=None):
    """
    Renders stroke sequence as an SVG image. (get from https://github.com/hardmaru/write-rnn-tensorflow)
...
def main(args):
    is_iamondb = False
    if "iamondb" in args.data_file:
        is_iamondb = True
        sample_ids = [1, 20, 150, 1000, 1222]
        factor = 10
    elif "deepwriting" in args.data_file:
        sample_ids = [1, 20, 150]
        factor = 0.001
    else:
    ...
    for sample_id in sample_ids:
        ...
        sample = dataset.prepare_for_visualization(stroke_sample)
...
if __name__ == '__main__':
    import argparse
    import os
    from dataset_ink import InkDataset as Dataset

    parser = argparse.ArgumentParser(prog='main')
    parser.add_argument('-D', '--data_file', action="store", dest="data_file", required=True, help='Input data path.')
    parser.add_argument('-O', '--out_dir', action="store", dest="out_dir", required=True, help='Output directory to save in.')
