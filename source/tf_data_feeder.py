...
class DataFeederTF(object):
    ...
    def __init__(self, dataset, num_epochs, batch_size=16, queue_capacity=512, shuffle=True, allow_smaller_final_batch=False):
        ...
        if shuffle:
            self.input_queue = tf.RandomShuffleQueue(queue_capacity, min_after_dequeue=int(queue_capacity/2), dtypes=self.dataset.sample_tf_type, names=self.dataset.sample_key)
        else:
            self.input_queue = tf.FIFOQueue(queue_capacity, dtypes=self.dataset.sample_tf_type, names=self.dataset.sample_key)
    ...
    def __enqueue(self, tf_session, tf_coord):
        ...
        while self.epoch <= self.num_epochs and not tf_coord.should_stop():
            try:
